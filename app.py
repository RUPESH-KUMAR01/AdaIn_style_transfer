...
# Streamlit UI
st.title("Real-time Style Transfer")
st.write("Upload a content image and a style image to apply the effect.")
...
if content_file and style_file:
    ...
    content_tensor = transform_image(content_img)
    style_tensor = transform_image(style_img)

    with torch.no_grad():
        output_tensor = model.decoder(adain(model.encode(content_tensor), model.encode(style_tensor)))
    ...
    with col3:
        st.image(output_image, caption="Styled Output", use_container_width=True)
